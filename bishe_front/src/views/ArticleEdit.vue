<template>
  ...
  <div id="article-create">
    ...
    <form>
      ...
      <div class="form-elem">
        <span>正文：</span>
        <textarea v-model="body" placeholder="输入正文" rows="20" cols="50"></textarea>
      </div>
      ...
      <div class="form-elem button">
        <button v-on:click.prevent="deleteArticle" style="background-color: darkred" class="position">删除</button>
      </div>
    </form>
  </div>
...
import {editArticle, getArticleDetail, getCategories} from "@/network/home";
...
export default {
  ...
  methods: {
      ...
      authorization().then(response => {
        if (response[0]) {
          ...
          editArticle(this.articleID, data, {
            ...
            this.$router.push({name: 'ArticleDetail', params: {id: response.id}})
          })
        } else {
          alert('令牌过期，请重新登录!')
        }
    ...
    deleteArticle() {
      ...
      authorization().then(response => {
        if (response[0]) {
          axios.delete('http://127.0.0.1:8000/api/articles/' + this.articleID + '/',
              {
